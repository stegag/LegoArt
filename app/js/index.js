...
function getProjectName() {
    ...
    if (!input) return "Lego";
    const name = input.value.trim();
    return name === "" ? "LegoArt" : name;
} 
    ...
    runStep4(async () => {
        ...
        for (var i = 0; i < totalPlates; i++) {
            ...
            if ((i + 1) % (isHighQuality ? 20 : 50) === 0) {
                pdf.save(`${projectName}-Instructions-Part-${numParts}.pdf`);
                numParts++;
                pdf = new jsPDF({
                    orientation: titlePageCanvas.width < titlePageCanvas.height ? "p" : "l",
                    unit: "mm",
        ...
        pdf.save(numParts > 1 ? `${projectName}-Instructions-Part-${numParts}.pdf` : `${projectName}-Instructions.pdf`);
